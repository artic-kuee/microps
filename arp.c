...
arp_cache_select(ip_addr_t pa)
{
    ...
    for (entry = caches; entry < tailof(caches); entry++) {
        if(entry->state != ARP_CACHE_STATE_FREE && entry->pa == pa) return entry;
    }
    return NULL;
}
...
arp_input(const uint8_t *data, size_t len, struct net_device *dev)
{
    ...
    if(msg->hdr.hln != ETHER_ADDR_LEN || ntoh16(msg->hdr.hrd) != ARP_HRD_ETHER){
        errorf("hardware addr type is not ethernet");
        return;
    }
    if(msg->hdr.pln != IP_ADDR_LEN || ntoh16(msg->hdr.pro) != ARP_PRO_IP){
        errorf("software addr type is not IP");
        return;
    }
