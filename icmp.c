...
icmp_input(const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst, struct ip_iface *iface)
{
    ...
    if(len < ICMP_HDR_SIZE){
        errorf("given icmp packet is too short");
        return;
    }
    ...
    case ICMP_TYPE_ECHO:
        ...
        icmp_output(ICMP_TYPE_ECHOREPLY, hdr->code, ntoh32(hdr->values),data + ICMP_HDR_SIZE, len - ICMP_HDR_SIZE, iface->unicast, src);
...
icmp_output(uint8_t type, uint8_t code, uint32_t values, const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst)
{
    ...
    return ip_output(IP_PROTOCOL_ICMP, buf, len + ICMP_HDR_SIZE, src, dst);
