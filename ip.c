...
ip_output(uint8_t protocol, const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst)
{
    ...
    if (src == IP_ADDR_ANY && dst == IP_ADDR_BROADCAST) {
        errorf("source address s required for broadcast addresses");
        return -1;
    } 
    route = ip_route_lookup(dst);
    if (!route) {
    ...
    if (src != IP_ADDR_ANY && src != iface->unicast) {
        ...
        errorf("%s", ip_addr_ntop(iface->unicast, addr,sizeof(addr)));
        return -1;
    }
...
ip_iface_register(struct net_device *dev, struct ip_iface *iface)
{
    ...
    if(!ip_route_add(iface->unicast & iface->netmask, iface->netmask, iface->broadcast, iface)) {
...
ip_protocol_register(uint8_t type, void (*handler)(const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst, struct ip_iface *iface))
{
    ...
    for(entry = protocols; entry; entry = entry->next){
        if(entry->type == type){
            errorf("this protocol is  already registered");
            return -1;
        }
    }
...
ip_input(const uint8_t *data, size_t len, struct net_device *dev)
{
    ...
    if((hdr->vhl >> 4) != IP_VERSION_IPV4) {
	    errorf("ip version not supported");
	    return;
    }
    hlen = (hdr->vhl & 0x0f)<<2;
    total = ntoh16(hdr->total);
    ...
    for(protocol = protocols; protocol; protocol = protocol->next){
        if(protocol->type == hdr->protocol){
            protocol->handler(data + hlen, len - hlen, hdr->src, hdr->dst, iface);
        }
    }
